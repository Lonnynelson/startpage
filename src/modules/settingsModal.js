...
export class FullSettingsModal {
  ...
  build() {
    ...
    const tabDefs = [
      ...
      { id: "fs-tab-categorized", label: "Categorized" },
    ...
    const categorizedPane = this.buildCategorizedPane();
    ...
    const footer = this._el("div", { className: "fs-footer" }, [
      this._el("p", {}, [
            this._el("strong", {}, [
          document.createTextNode("© Lonny Nelson "),
            yearSpan,
        ]),
      ]),
      this._el("p", { textContent: "Based on YourDynamicDashboard V3.0.0" }),
    ]);
  ...
  // Categorized links settings
  buildCategorizedPane() {
    const pane = this._el("div");

    const listContainer = this._el("div", {
      className: "fs-shortcuts-list",
      id: "fs-categorized-editor-list",
    });
    this.els.fsCategorizedList = listContainer;
    pane.appendChild(
      this._section("Categorized Items", [listContainer]),
    );

    const form = this._el("form", { id: "fs-add-categorized-form" });
    form.appendChild(
      this._el("h3", {
        className: "settings-header",
        textContent: "Add New Categorized Item",
      }),
    );
    ...
    const catInput = this._el("input", {
      type: "text",
      id: "fs-cat-category-input",
      placeholder: "Category (e.g., AS)",
      maxlength: "35",
      required: "",
    });
    const nameInput = this._el("input", {
      type: "text",
      id: "fs-cat-name-input",
      placeholder: "Name (e.g., Time Clock)",
      maxlength: "35",
      required: "",
    });
    const urlInput = this._el("input", {
    ...
    this.els.fsCategorizedForm = form;
    this.els.fsCatCategoryInput = catInput;
    this.els.fsCatNameInput = nameInput;
    this.els.fsCatUrlInput = urlInput;
  ...
  buildDataPane() {
    ...
    const catPageBtn = this._el("button", {
      type: "button",
      className: "settings-button",
      textContent: "Open Categorized Tab",
    });
    catPageBtn.addEventListener("click", () => this.openTab("fs-tab-categorized"));
    pane.appendChild(
      this._section("Categorized Items Manager", [
        this._row(
          "Page 2 Categorized Links",
          "View, edit, or add categorized links and favicons for Page 2.",
          catPageBtn,
        ),
      ]),
    );
  ...
  bindCoreEvents() {
    ...
    if (this.els.fsCategorizedForm) {
      this.els.fsCategorizedForm.addEventListener("submit", (e) => {
        e.preventDefault();
        const cat = this.els.fsCatCategoryInput?.value.trim();
        const name = this.els.fsCatNameInput?.value.trim();
        let url = this.els.fsCatUrlInput?.value.trim();
        ...
        const items = this._getCategorizedItems();
        items.push({
          category: cat,
          name,
          url,
        ...
        this._saveCategorizedItems(items);

        if (this.els.fsCatCategoryInput) this.els.fsCatCategoryInput.value = "";
        if (this.els.fsCatNameInput) this.els.fsCatNameInput.value = "";
        if (this.els.fsCatUrlInput) this.els.fsCatUrlInput.value = "";
      });
    }

    window.addEventListener("storage", (e) => {
      if (e.key === "userCategorizedList" || e.key === null) {
        if (this.isOpen) this._renderCategorizedEditor();
      }
    });
  ...
  _saveCategorizedItems(items) {
    localStorage.setItem("userCategorizedList", JSON.stringify(items));
    this._renderCategorizedEditor();
    window.__categorizedManagerInstance?.render();
  }

  _reorderCategorizedItems(fromIndex, toIndex) {
    const items = this._getCategorizedItems();
    if (
      fromIndex < 0 ||
  ...
  _renderCategorizedEditor() {
    const list = this.els.fsCategorizedList;
    if (!list) return;
    list.replaceChildren();
    ...
    if (items.length === 0) {
      list.appendChild(
        this._el("p", {
          ...
          textContent: "No categorized items added yet.",
        }),
      );
      return;
    }
    ...
    items.forEach((item, index) => {
      ...
      const triggerSave = () => {
        ...
        const currentItems = this._getCategorizedItems();
        if (currentItems[index]) {
          currentItems[index].category = catVal;
          currentItems[index].name = nameVal;
          currentItems[index].url = urlVal;
          currentItems[index].icon = `https://www.google.com/s2/favicons?sz=64&domain=${encodeURIComponent(urlVal)}`;
          this._saveCategorizedItems(currentItems);
        }
      };
      ...
      div.addEventListener("drop", (e) => {
        ...
        if (!isNaN(from) && from !== index) {
          this._reorderCategorizedItems(from, index);
        }
      });
