...
export class SettingsManager {
  ...
  async backup() {
    try {
      ...
      const rawFlipbookMenu = JSON.parse(
        localStorage.getItem("flipbookMenu") || "[]",
      );
      const exportedFlipbookMenu = await Promise.all(
        rawFlipbookMenu.map(async (item) => {
          const itemCopy = { ...item };
          if (itemCopy.id) {
            try {
              const pdfBlob = await secondStorage.getPdfBlob(itemCopy.id);
              if (pdfBlob) {
                itemCopy.dataUrl = await this.blobToDataUrl(pdfBlob);
              }
            } catch (err) {
              console.warn("Could not read PDF Blob from IndexedDB during backup:", err);
            }
          }
          return itemCopy;
        }),
      ...
      const data = {
        ...
        userCategorizedList: JSON.parse(
          localStorage.getItem("userCategorizedList") || "[]",
        ),
        flipbookMenu: exportedFlipbookMenu,
        dndPageLinks: JSON.parse(localStorage.getItem("dndPageLinks") || "[]"),
    ...
    try {
      ...
      if (data) {
        if (Array.isArray(data.userShortcuts)) {
          rawEntries.userShortcuts = JSON.stringify(data.userShortcuts);
        }
        if (Array.isArray(data.userCategorizedList)) {
          rawEntries.userCategorizedList = JSON.stringify(
            data.userCategorizedList,
          );
        }
        if (Array.isArray(data.flipbookMenu)) {
          const restoredFlipbookMenu = await Promise.all(
            data.flipbookMenu.map(async (item) => {
              const itemCopy = { ...item };
              if (
                itemCopy.dataUrl &&
                typeof itemCopy.dataUrl === "string" &&
                itemCopy.dataUrl.startsWith("data:")
              ) {
                try {
                  const pdfBlob = await this.dataUrlToBlob(itemCopy.dataUrl);
                  const pdfId =
                    itemCopy.id ||
                    `pdf_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
                  itemCopy.id = pdfId;
                  await secondStorage.savePdfBlob(pdfId, pdfBlob);
                  delete itemCopy.dataUrl;
                } catch (err) {
                  console.warn("Could not save restored PDF Blob to IndexedDB:", err);
                }
              }
              return itemCopy;
            }),
          );
          rawEntries.flipbookMenu = JSON.stringify(restoredFlipbookMenu);
        }
        if (Array.isArray(data.dndPageLinks)) {
          rawEntries.dndPageLinks = JSON.stringify(data.dndPageLinks);
        }
      }

      const entries = validateYddStorageEntries(rawEntries);
