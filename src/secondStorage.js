...
export const secondStorage = {
  ...
  // PDF blob storage API (for large PDFs)
  savePdfBlob(id, blob) {
    return enqueueMutation(() =>
      runTransaction("readwrite", (store) => store.put(blob, `pdf_${id}`))
    );
  },

  async getPdfBlob(id) {
    await mutationQueue;
    return runTransaction("readonly", (store) => store.get(`pdf_${id}`));
  },

  deletePdfBlob(id) {
    return enqueueMutation(() =>
      runTransaction("readwrite", (store) => store.delete(`pdf_${id}`))
    );
  },
...
if (typeof window !== "undefined") {
  window.secondStorage = secondStorage;
